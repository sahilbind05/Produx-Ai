...
function StatCard({ label, value, unit, sub, accent }) {
  return (
    <div className="bg-ink-800 border border-ink-700 rounded-sm p-3.5 md:p-4">
      <p className="text-[10px] md:text-[11px] text-ink-500 uppercase tracking-widest font-display mb-2">{label}</p>
      <p className="font-display text-2xl md:text-3xl font-bold" style={{ color: accent }}>
        {value}<span className="text-sm text-ink-500 ml-1 font-body">{unit}</span>
      </p>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <StatCard label="Completion" value={completionRate} unit="%" sub={`${completedTasks}/${totalTasks} tasks`} accent={JADE} />
        <StatCard label="Active Goals" value={activeGoals} unit="" sub={`${goals.length} total`} accent={SKY} />
        <StatCard label="Avg Streak" value={avgStreak} unit="days" sub={`${habits.length} habits`} accent={AMBER} />
        <StatCard label="This Week" value={weekTotal} unit="done" sub="tasks completed" accent={ROSE} />
      </div>

      {/* Weekly area chart */}
      <div className="bg-ink-800 border border-ink-700 rounded-sm p-3.5 md:p-4 mb-4">
        ...
        <ResponsiveContainer width="100%" height={160}>
          <AreaChart data={weeklyData} margin={{ top: 5, right: 5, bottom: 0, left: -25 }}>
            ...
            <XAxis dataKey="day" tick={{ fontSize: 10, fill: "#888", fontFamily: "DM Sans" }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: 10, fill: "#888", fontFamily: "DM Sans" }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={tooltipStyle} />
            <Area type="monotone" dataKey="completed" stroke={JADE} fill="url(#jadeGrad)" strokeWidth={2} name="Completed"
              dot={{ r: 3, fill: JADE, strokeWidth: 0 }} />
            <Area type="monotone" dataKey="total" stroke="#333" fill="none" strokeWidth={1} strokeDasharray="4 2" name="Total" />
          </AreaChart>
        </ResponsiveContainer>
      </div>

      {/* Bottom charts — stacked on mobile, side by side on md+ */}
        ...
        <div className="bg-ink-800 border border-ink-700 rounded-sm p-3.5 md:p-4">
          ...
          <ResponsiveContainer width="100%" height={130}>
            <BarChart data={priorityData} margin={{ top: 5, right: 5, bottom: 0, left: -25 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#252525" vertical={false} />
              <XAxis dataKey="name" tick={{ fontSize: 10, fill: "#888", fontFamily: "DM Sans" }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 10, fill: "#888", fontFamily: "DM Sans" }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={tooltipStyle} />
